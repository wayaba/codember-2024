const recordsList = `PYmt8hd,jdcrLI,jdcrli@gmail.com,27,London
...
const validateRecords = (records) =>  {
    ...
    records.split('\n').forEach(record=>{
        const [ id, username, email, age, location] = record.split(',')

        let isValid = true

        if(!isValidField(id)) isValid = false
        if(isValid && !isValidField(username)) isValid = false
        if(isValid && !isEmailValid(email)) isValid = false
        if(isValid && !isAgeValid(age)) isValid = false
        if(isValid && !isLocationValid(location)) isValid = false

        
        if(!isValid){
            hackMessage += username.charAt(0)
        }
        
    })
    
    console.log(hackMessage)
}

const isValidField = (text) =>{
