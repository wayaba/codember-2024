...
function parsePassowrds(passwords) {
  let errorPass = []

  passwords.split('\n').forEach((line) => {
    const [range, letterdirty, password] = line.split(' ')
    const letter = letterdirty.replace(/:/g, '')
    const matchesCount = password.split('').filter(function (char) {
      return char === letter
    }).length

    const [min, max] = range.split('-')
    if (!(matchesCount >= min && matchesCount <= max)) errorPass.push(password)
  })
  console.log('Pass errorea (42)', errorPass[41])
  console.log('Pass administrador (13)', errorPass[12])
}
